...
AL sinyali geldiğinde Telegram bildirimi gönderir
"""
...
# ============================================================
# AYARLAR (Telegram bilgileri GitHub Secrets'tan okunacak)
# ============================================================
...
def tarama():
    ...
    for i, sembol in enumerate(HISSE_LISTESI):
        ...
        if onceki_pozisyon is not None and onceki_pozisyon != sonuc['pozisyon']:
            if sonuc['pozisyon'] == 'LONG':
                yeni_al_sinyalleri.append(sonuc)
            else:
                yeni_sat_sinyalleri.append(sonuc)
    ...
    print(f"\nTarama tamamlandi.")
    print(f"Yeni AL sinyalleri: {len(yeni_al_sinyalleri)}")
    print(f"Yeni SAT sinyalleri: {len(yeni_sat_sinyalleri)}")

    if yeni_al_sinyalleri or yeni_sat_sinyalleri:
        mesaj = f"<b>BIST CE Sinyal Degisikligi</b>\n"
        mesaj += f"Saat: {datetime.now().strftime('%d.%m.%Y %H:%M')}\n\n"

        if yeni_al_sinyalleri:
            mesaj += "AL Sinyali (LONG'a gecti):\n"
            for s in yeni_al_sinyalleri:
                mesaj += f"  {s['sembol']} - {s['fiyat']} TL\n"

        if yeni_sat_sinyalleri:
            mesaj += "\nSAT Sinyali (SHORT'a gecti):\n"
            for s in yeni_sat_sinyalleri:
                mesaj += f"  {s['sembol']} - {s['fiyat']} TL\n"
